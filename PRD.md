...
| Text anchored differently in an aligned row | `margin_alignment.text_anchor_mismatch` | Boxes that line up while the text inside them does not: three or more same-height boxes sharing a top edge within the intent window, where a minority anchors its text to a different edge of the box (`a:bodyPr/@anchor`). The one alignment defect the box-edge rules cannot see — aligning the box and aligning the text inside it are two different things (design lead, 20/08/2026). | The anchor is an attribute the deck STATES, so this stays inside the module's box-versus-text rule: no glyph is measured. Three exclusions, each because the comparison would otherwise be a guess — a PLACEHOLDER (its anchor comes from the master, which this tool follows and never overrides), a box carrying `a:spAutoFit` (its height tracks its text, so the anchor draws nothing), and an empty box. Requires one height class (within 1.25×) and a clear majority, the same evidence bar as the edge clusters. Flag-only and permanently outside `FIXABLE_ISSUES`: a deliberately bottom-anchored caption under a row of top-anchored ones is a real design. Severity `warning`. |
...
- `geometry.safe_zone_margins_emu` (left/right/top/bottom): the safe-zone margins compared against effective shape edges. Projected from the best statement the master makes, in this order (`style_spec_source.grid_source` records which): the **presentation space**, a rectangle the designer draws on the master; then the drawing **guides**; then the master's content **placeholders**. The rectangle outranks the guides because it needs no interpreting - a master can carry an outer page margin, a column grid, a header band and a bleed line, and choosing among them is a guess (design lead, 21/08/2026: "some cases have multiple margins, so presentation space is safer"). The reserved header band is still read from the guides: one rectangle cannot state a pair of lines inside itself.

  **How the rectangle is marked.** Either by NAME ("Presentation space", "Content area") or by ALT TEXT — `ToolsToo_PS`, which is what the ToolsToo add-in stamps when a designer sets a presentation space with it, leaving the shape name as PowerPoint made it ("Rectangle 2"). The alt text is the marker the design team actually produces, and reading names only meant the client master's own frame was never found: every read fell through to the guides. Where both exist the alt text wins — a name is typed and survives copying, so it can be a leftover, while the alt text is on the rectangle the add-in is currently driving. Two markers that disagree are both reported rather than resolved by document order (`qc.stylespec.space_marker`).

  **What the rectangle is a frame AROUND** (`grid.space_states`). Designers draw it both ways and the two mean different things. Around the whole page, its top IS the page's top margin. Around the **body** — which is how the client master draws it, its top landing on the master's own body placeholder to the EMU at 1.90in, 1.45in below the page's top margin — its top is where CONTENT begins, and the header lives above it by design. Which one a given rectangle is, is a fact and not a guess: a frame starting at or below the master's own title and subtitle boxes excludes the header by construction. A body frame contributes its sides and its floor to `safe_zone_margins_emu`, contributes its top to `body_band_emu.body_top` (when the guides do not already state it), and leaves the page's top margin to the next-best statement. Read as a page margin instead, that top would become the deck's safe-zone ceiling and every title on every slide — all of which sit above it, where the master puts them — would be reported as breaking a margin the designer never drew there.
...
**The presentation space is written into the output deck** (`qc.pspace.ensure_presentation_space`), on every slide master it ends up with, as an invisible rectangle carrying the alt text `ToolsToo_PS`. Applying a master copies the design, so a marker travels — but only to where the designer drew it. On the client master that is ONE layout ("USE THIS"), so a slide sitting on any other layout inherits no frame, and a straggler slide keeps a design that never had one; a designer opening that deck and reaching for ToolsToo's align-to-presentation-space finds nothing to align to, on exactly the slides that most need it. The master is where a frame governing the whole deck belongs, and it is the first place the read looks, so a re-audit of the output finds the same frame the format was seated against rather than a weaker guess from the guides.

The geometry comes from the OUTPUT deck wherever the applied design already states it, never from the submitted master file: PowerPoint resizes a loaded design to the deck's slide size, so the file it just wrote is the only place the frame's real numbers are, and taking them from the master would plant a 16:9 rectangle on a 4:3 deck and call it the frame. Only when the output states none anywhere is the master's box used, scaled, and the scaling reported. This runs BEFORE the content migration, so every slide — stragglers included — is seated on the frame the deck now carries. The result page states what was added and where the numbers came from, because "did the presentation space come across?" must be answerable without opening master view.
